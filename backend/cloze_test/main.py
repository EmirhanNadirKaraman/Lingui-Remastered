...
def get_magic_sentences():
    ...
    cursor.execute("WITH sentences_with_word AS ("
                   ...
                   "    word_count.cnt - COALESCE(known_words.cnt, 0), "
                   "    SWW.sentence_id, SWW.content, SWW.video "
                   ...
                   "WHERE SWW.content ~ %s "
                   "ORDER BY word_count.cnt - COALESCE(known_words.cnt, 0) "
                   "OFFSET ((%s - 1) * %s) ROWS "
                   "LIMIT %s",
                   (unknown_word_id, language, uid, regex, page, rows_per_page, rows_per_page))

    sentences = [{'unknown_count': sentence[0],
                  'sentence_id': sentence[1],
                  'content': sentence[2],
                  'video_properties': sentence[3]} for sentence in cursor.fetchall()]
...
def get_cloze_questions():
    ...
    # word_arr = [{"word": "cat", "pos": "NOUN"}, {"word": "eat", "pos": "VERB"}, ...]
    ...
    for word in word_arr:
        first_str = """
        (select WTS.word, S1.sentence, S2.sentence, WTS.pos, WT.word_id
        from 
        ...
        where 
            ...
            S1.language = %s and 
            S2.language = %s and
            WTS.sentence_id = S1.sentence_id and 
            S1.sentence_id = SP.second_id and
            S2.sentence_id = SP.first_id
        limit 1)"""
        ...
        (select WTS.word, S1.sentence, S2.sentence, WTS.pos, WT.word_id
            ...
            where 
                ...
                S2.language = %s and
                WTS.sentence_id = S1.sentence_id and 
                S1.sentence_id = SP.first_id and
                S2.sentence_id = SP.second_id
            limit 1)"""
